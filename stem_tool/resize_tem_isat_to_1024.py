...
DEFAULT_IMAGE_DIR = SCRIPT_ROOT / "data" / "TEM" / "swinir_real_sr_x2_resize2048"
DEFAULT_LABEL_DIR = SCRIPT_ROOT / "data" / "TEM" / "zwl_100_label"
DEFAULT_OUTPUT_IMAGE_DIR = SCRIPT_ROOT / "data" / "TEM" / "swinir_real_sr_x2_resize512"
DEFAULT_OUTPUT_LABEL_DIR = SCRIPT_ROOT / "data" / "TEM" / "zwl_100_label_512"
...
def resize_image(image_path: Path, output_path: Path, width: int, height: int) -> tuple[int, int]:
...
def process_annotation(
    ...
    src_w = int(info.get("width", target_w)) if isinstance(info, dict) else target_w
    src_h = int(info.get("height", target_h)) if isinstance(info, dict) else target_h
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument("--width", type=int, default=DEFAULT_TARGET_WIDTH, help="Target width.")
    parser.add_argument("--height", type=int, default=DEFAULT_TARGET_HEIGHT, help="Target height.")
...
def main() -> None:
    ...
    for image_path in image_paths:
        ...
        resize_image(image_path, output_path, target_w, target_h)
        resized_images += 1
